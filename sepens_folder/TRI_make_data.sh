...
if [[ -f ./sepsis_partition-G.tsv && \
      -f ./sepsis_partition-H.tsv && \
      -f ./sepsis_partition-I.tsv
  ]]; then
    ...
    # Split 0
    CNT=$(($(wc -l sepsis_partition-G.tsv | awk '{print $1}')-1))
    tail -n $CNT sepsis_partition-G.tsv > "$tmpfile"
    sort -k1n "$tmpfile" > train.dat
   
    CNT=$(($(wc -l sepsis_partition-H.tsv | awk '{print $1}')-1))
    tail -n $CNT sepsis_partition-H.tsv > dev.dat
   
    CNT=$(($(wc -l sepsis_partition-I.tsv | awk '{print $1}')-1))
    tail -n $CNT sepsis_partition-I.tsv > test.dat

    elif [ $SPLIT=1 ]; then
    
    # Split 1
    CNT=$(($(wc -l sepsis_partition-H.tsv | awk '{print $1}')-1))
    tail -n $CNT sepsis_partition-H.tsv > "$tmpfile"
    sort -k1n "$tmpfile" > train.dat
   
    CNT=$(($(wc -l sepsis_partition-G.tsv | awk '{print $1}')-1))
    tail -n $CNT sepsis_partition-G.tsv > dev.dat
