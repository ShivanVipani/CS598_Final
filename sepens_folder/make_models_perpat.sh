...
for i in `cut -f2 sorted_filter_train_ids.dat`
...
do
  ...
  python sepens/main_ts.py --data "data/data_$i" --save "models/model_$i.pt" \
                    --cuda --epochs 20 --min_epochs 10 \
                    2>&1 | tee "logs/training_sepsis_$i.log"
  python sepens/predict_ts.py --data "data/data_$i" \
